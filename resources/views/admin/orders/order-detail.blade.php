...
<div class="max-w-5xl mx-auto bg-white shadow-md rounded-2xl border border-gray-100 p-8">
        ...
        <div class="space-y-2">
            ...
            <p><span class="font-semibold">Status:</span>
                <span class="
                    ...
                    @elseif(strtolower($order->status) == 'dibatalkan') bg-red-100 text-red-600
                    @endif">
    ...
    {{-- Aksi: Konfirmasi / Batalkan --}}
    @if(!in_array(strtolower($order->status), ['selesai']))
    <div class="border-t pt-6">
        <h3 class="font-semibold text-gray-700 text-lg mb-4">Menunggu Konfirmasi</h3>
        <form action="{{ route('admin.orders.update', $order->id) }}" method="POST" class="flex flex-wrap gap-3">
            ...
            {{-- Tombol Batalkan --}}
            @if(!in_array(strtolower($order->status), ['dibatalkan', 'selesai']))
